...
======================================================================
Reproduces the 4-variable coupled system benchmark from the README
over N = 1,000,000 iterations across 3 tiers:
...
def benchmark_native_c(N: int = 1_000_000):
    if not shutil.which("gcc"):
        print("[!] GCC compiler not found in PATH; skipping C benchmark.")
        return
...
int main() {{
    ...
    #define START_TIMER() QueryPerformanceCounter(&t0)
    #define STOP_TIMER()  QueryPerformanceCounter(&t1)
    #define CALC_MS()     get_time_ms(t0, t1, freq)
#else
    struct timespec t0, t1;
    #define START_TIMER() clock_gettime(CLOCK_MONOTONIC, &t0)
    #define STOP_TIMER()  clock_gettime(CLOCK_MONOTONIC, &t1)
    #define CALC_MS()     get_time_ms(t0, t1)
#endif
