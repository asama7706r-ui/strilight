...
from strilight.engine.vsa.models import SpatiotemporalCoordinate

logger = logging.getLogger("strilight.extensions.stack_engine")


def _is_timestamp_le(t1: Any, t2: Any) -> bool:
    """Helper to compare timestamps whether int or SpatiotemporalCoordinate."""
    if t1 is None or t2 is None or t2 == 0:
        return True
    try:
        return t1 <= t2
    except TypeError:
        if isinstance(t1, SpatiotemporalCoordinate) and isinstance(t2, int):
            return isinstance(t1.k, int) and t1.k <= t2
        elif isinstance(t1, int) and isinstance(t2, SpatiotemporalCoordinate):
            return isinstance(t2.k, int) and t1 <= t2.k
        return True
...
class StackByteCell:
    ...
    def __init__(
        ...
        timestamp: Union[int, SpatiotemporalCoordinate] = 0,
    ...
    def read_bytes(
        ...
        for i in range(size_bytes):
            ...
            if is_concrete and concrete_base is not None:
                ...
                if c_addr in self.cells and (tick == 0 or _is_timestamp_le(self.cells[c_addr].timestamp, tick)):
            ...
            if cell is None:
                ...
                for w_addr, w_cell, w_sz in reversed(self.symbolic_writes):
                    if tick != 0 and not _is_timestamp_le(w_cell.timestamp, tick):
                        continue
