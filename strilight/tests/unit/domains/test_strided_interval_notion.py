...
import math
import pytest
from strilight.engine.domains import StridedInterval, Interval, DisjointIntervalSet
...
# ==============================================================================
# TEST 6: Singleton Constant Stride Zero & Pointer Alignment Preservation
# ==============================================================================
def test_constant_singleton_stride_zero():
    """
    Tests that singleton constants (min_val == max_val) default to stride=0,
    preserving pointer alignment under addition and scaling stride under multiplication.
    """
    # 1. Automatic stride 0 for constant point
    c_const = Interval(5, 5)
    assert c_const.stride == 0

    # 2. Addition preserves stride: gcd(4, 0) == 4
    idx = Interval(10, 50, stride=4)
    shifted = idx.add(c_const)
    assert shifted.min_val == 15
    assert shifted.max_val == 55
    assert shifted.stride == 4

    # 3. Multiplication scales stride: gcd(10*0, 8*4, 4*0) == 32
    scale_const = Interval(8, 8)
    assert scale_const.stride == 0
    scaled = idx.mul(scale_const)
    assert scaled.min_val == 80
    assert scaled.max_val == 400
    assert scaled.stride == 32

    # 4. Explicit stride is respected when explicitly provided
    dense_point = Interval(5, 5, stride=1)
    assert dense_point.stride == 1
