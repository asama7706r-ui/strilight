...
def test_parse_c_pragma_contract():
    ...
    # 3. Multi-target / Multi-entity contract
    clause3 = 'target(pos, vel, energy) model(coupled)'
    contract3 = parse_c_pragma_contract(clause3)
    assert contract3.target == "pos, vel, energy"
    assert contract3.targets == ["pos", "vel", "energy"]
    assert contract3.model == "coupled"

    # 4. Empty clause
    contract4 = parse_c_pragma_contract("")
    assert contract4.target is None
    assert contract4.targets == []
    assert contract4.include_file is None
    assert contract4.is_empty()
