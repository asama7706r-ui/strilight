...
4. C nested loop lifting and induction composition matching nested_loops.c.
...
def test_c_nested_loop_lifting_induction_pattern():
    """
    Tests lifting the exact nested loop structure from nested_loops.c:
        for (int i = 0; i < 1000; i++) {
            for (int j = 0; j < 5; j++) {
                acc += 15 + j;
            }
    ...
    acc_c_func = SourceLifter.accelerate_c(c_code, in_place=False, func_name="fast_nested_loop")
    assert "fast_nested_loop_result_t fast_nested_loop(uint64_t N, uint64_t acc)" in acc_c_func
